...
async def collect_art(
    request: Request, art_id: int, db: AsyncSession = Depends(get_db)
):
    """
    收藏艺术作品
    ...
    try:
        ...
        collect_query = select(VigaCollect).where(
            VigaCollect.art_id == art_id, VigaCollect.user_id == user_id
        )
        ...
        if existing_collect:
            ...
            new_collect = VigaCollect(
                author_id=author_id, user_id=user_id, art_id=art_id
            )
            db.add(new_collect)
            await db.commit()
...
async def share_art(request: Request, art_id: int, db: AsyncSession = Depends(get_db)):
    """
    分享艺术作品
    ...
    try:
        ...
        share_query = select(VigaShare).where(
            VigaShare.art_id == art_id, VigaShare.user_id == user_id
        )
        ...
        # 创建分享记录
        new_share = VigaShare(author_id=author_id, user_id=user_id, art_id=art_id)
        db.add(new_share)
        await db.commit()
