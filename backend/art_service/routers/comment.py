...
@router.get("/comments/list", response_model=CommentListResponse, summary="分页获取评论列表")
async def get_comments(
    ...
    try:
        ...
        stmt = select(VigaComment, VigaUsers.username).join(
            VigaUsers, VigaComment.user_id == VigaUsers.id, isouter=True
        ).where(VigaComment.art_id == art_id).offset(offset).limit(page_size)
...
@router.delete("/comments/{comment_id}", response_model=CommentResponse, summary="删除评论")
