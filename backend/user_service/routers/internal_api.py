...
from schemas.userinfo import UserInfoRequestOut,UserInfoListRequestIn,UserInfoListRequestOut
...
async def get_user_list(request:UserInfoListRequestIn, db: AsyncSession = Depends(get_db)): 
    # 业务逻辑：根据IDs查询用户#
    users = await get_users_by_ids(db=db, 
        user_ids=request.ids)
    return UserInfoListRequestOut.create_response(
        users=users,
        total_count=len(users)  # 如果是精确查询，总数等于返回数
    )