    ...
    try:
        ...
        async for message in websocket:
            ...
            if jsonData['type'] == "login":
                """
                1, 选择本地账号, 选择登录节点(在选择的过程中,需要明示租赁费用)
                2, 若目标节点无本用户信息, 则同步本地个人区块到远程节点, 远程节点验证区块准确性, 验证不通过, 则不允许登录
                3, 若目标节点有本用户信息, 则允许登录
                4, 互相同步最新区块, 本地旧则同步远程(比如换手机情况),本地验证区块, 远程旧则同步本地到远程(内网使用情况)
                5, !!!考虑合并信息的情况, 有可能内网网聊到100个区块的时候, 另外一个设备在外网也聊到106个区块.
                """
                pass
