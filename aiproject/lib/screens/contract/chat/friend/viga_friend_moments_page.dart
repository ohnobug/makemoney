...
import 'package:vigaviga/widgets/viga_spans.dart';
...
class VigaFriendmomentsPage extends StatefulWidget {
  const VigaFriendmomentsPage({super.key});

  @override
  State<VigaFriendmomentsPage> createState() => _VigaFriendmoments();
}

// 朋友圈页面的状态管理类
// with TickerProviderStateMixin 是为了让 State 类能够提供 Ticker，这是驱动动画所必需的。
class _VigaFriendmoments extends State<VigaFriendmomentsPage>
  ...
  void _scrollListener() {
    ...
    double beginPosition =
        context.read<VigaSystemCubit>().state.statusHeight + 450.w;
...
class _TweetWidgetState extends State<TweetWidget> {
  ...
  List<InlineSpan> _buildLikeSpans(
    ...
    for (int i = 0; i < likes.length; i++) {
      ...
      // 使用 VigaBuildspan 来创建可点击的文本片段
      spans.addAll(VigaBuildspan(
        context,
        name,
